...
pub fn encrypt_key_file(key_name: &str, key_bytes: &[u8]) -> Result<(), EncryptionError> {
    ...
    match fs::write(&encrypted_path, &encrypted_data) {
        Ok(_) => println!("Debug: Encrypted key file written: {:?}", encrypted_path),
        Err(e) => {
            println!("Debug: Failed to write encrypted key file: {:?}", e);
            return Err(EncryptionError::IoError(e));
        }
    }

    match fs::remove_file(&key_path) {
        Ok(_) => println!("Debug: Original key file removed: {:?}", key_path),
        Err(e) => {
            println!("Debug: Failed to remove original key file: {:?}", e);
            return Err(EncryptionError::IoError(e));
        }
    }
